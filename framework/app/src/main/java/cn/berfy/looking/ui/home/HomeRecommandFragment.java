...
import cn.berfy.looking.adapter.home.HomeRecommandListAdapter;
 ...
 * 首页推荐列表
 */
public class HomeRecommandFragment extends BaseFragment implements RefreshableView.RefreshListener, FooterListview.OnRefreshListener, AdapterView.OnItemClickListener {

    private FooterListview mListview;
    private RefreshableView mRefreshableView;
    private HomeRecommandListAdapter mAdapter;
    ...
    protected void initView() {
        ...
        mAdapter = new HomeRecommandListAdapter(mContext);
    ...
    private void getData(boolean isCross) {
        ...
        List<ContentBean> contentBeens = TabContent.getInstances().getAllData(PAGE, PAGESIZE, true);
    ...
    public void onResume() {
        super.onResume();
        if (!TextUtils.isEmpty(TempSharedData.getInstance(mContext).getData(Constants.TEMP_REFRESH_HOME_RECOMMAND))) {
            getData(false);
            TempSharedData.getInstance(mContext).save(Constants.TEMP_REFRESH_HOME_RECOMMAND, "");
        }
    }
